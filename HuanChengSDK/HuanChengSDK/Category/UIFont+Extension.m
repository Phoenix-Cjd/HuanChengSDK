...
+ (UIFont *)adjustFontOfSize:(CGFloat)fontSize {
    return [UIFont adjustFontOfSize:floor(fontSize * MIN(kScreenWidth, kScreenHeight) / 375.f)];
}

+ (UIFont *)adjustFontWithName:(NSString *)fontName size:(CGFloat)fontSize {
    return [UIFont adjustFontWithName:fontName size:floor(fontSize * MIN(kScreenWidth, kScreenHeight) / 375.f)];
}
