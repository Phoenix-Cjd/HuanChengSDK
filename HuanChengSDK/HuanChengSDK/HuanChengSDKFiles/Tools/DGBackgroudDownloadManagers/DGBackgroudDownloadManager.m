...
/**
 ...
 @param url 下载的url
 @param destinationPath 自定义的缓存路径
 */
- (void)downloadWithUrl:(NSString *)url destinationPath:(NSString *)destinationPath process:(DownloadProgressBlock)process completion:(DownloadCompleteBlock)completion failure:(DownloadErrorBlock)failure {
    if (url.length == 0) {
        return;
    }
    // 判断缓存中是否存在,说明已经下载过了
    NSString *cachePath = destinationPath;
    if ([[NSFileManager defaultManager] attributesOfItemAtPath:cachePath error:nil].fileSize > 0) {
    ...
    if (completion) {
        model.downloadCompleteBlcok = completion;
    }
    model.downloadProgressBlock = process;
    if (failure) {
        model.downloadErrorBlock = failure;
    }
...
/// 不验证是否下载过了 如果下载过了就覆盖
- (void)downloadReplaceWithUrl:(NSString *)url destinationPath:(NSString *)destinationPath process:(DownloadProgressBlock)process completion:(DownloadCompleteBlock)completion failure:(DownloadErrorBlock)failure {
    if (url.length == 0) {
        return;
    }
    NSString *cachePath = destinationPath;
...
//        [DGBackgroudDownloadCacheManager saveDownloadModel:weakModel];
        ...
        if (weakModel.downloadCompleteBlcok) {
            weakModel.downloadCompleteBlcok(weakModel.cachePath);
        }
