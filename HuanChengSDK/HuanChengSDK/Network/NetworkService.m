...
+ (NSDictionary *)builderHttpHeader {
    ...
    if (token.length > 0) {
//        sysInfoDic[@"Authorization"] = USERMANAGER.token;
    }
    ...
    if (deviceId) {
        [sysInfoDic setObject:deviceId forKey:@"deviceId"];
    }
    NSString *str = [NSString stringWithFormat:@"%ld",[NSDate getCurrentStamp]];
    [sysInfoDic setObject:str forKey:@"st"];
    
    NSString *randStr = [self getRandStringWithLength:6];
    [sysInfoDic setObject:randStr forKey:@"sr"];
    
    NSString *ss = [NSString stringWithFormat:@"%@%@Ie89igOtoSK4FeijWsQu7If3z93KtFCm",str,randStr];
    [sysInfoDic setObject:[AESTool MD5EncryptUpper:ss] forKey:@"ss"];
    return sysInfoDic;
}
...
+ (NSString *)getRandStringWithLength:(int)length {
    NSString *sourceStr = @"abcdefghijklmnopqrstuvwxyzABCDEFGHIZKLMNOPQRSTUVWXYZ";
